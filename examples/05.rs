...
struct Transaction {
    compensation_logs: Vec<CompensationLog>,
    dirty_pages_ids: Vec<u8>,
}

impl Transaction {
    fn new() -> Self {
        Self {
            compensation_logs: Vec::new(),
            dirty_pages_ids: Vec::new(),
        }
    }
    fn log_insert(&mut self, page_id: u8, slot_id: u8) {
        self.compensation_logs
            .push(CompensationLog::Insert(InsertCompensationLog {
                page_id,
                slot_id,
            }));
        self.dirty_pages_ids.push(page_id);
    }
}

enum CompensationLog {
    Insert(InsertCompensationLog),
}

struct InsertCompensationLog {
    page_id: u8,
    slot_id: u8,
}
...
impl Page {
    ...
    fn insert_tuple(&mut self, tuple: u8, transaction: &mut Transaction) {
        let slot_id = self.tuple_length();
        transaction.log_insert(self.page_id(), slot_id);
        self.bytes[slot_id as usize + Self::HEADER_SIZE] = tuple;
        self.bytes[1] += 1;
    }
    fn rollback_insert(&mut self, slot_id: u8) {
        self.bytes[1] -= 1;
...
impl BufferPoolManager {
    ...
    fn unpin_page(&mut self, page_id: u8) {
    ...
    fn flush_page(&mut self, page_id: u8) {
        let frame_id = *self.page_frame_table.get(&page_id).unwrap();
        self.page_manager
            .write_page(&self.frames[frame_id].page.read().unwrap());
    }
}
...
impl Database {
    ...
    fn commit(&mut self, _transaction: &mut Transaction) {
        for page_id in &_transaction.dirty_pages_ids {
            self.buffer_pool_manager.flush_page(*page_id);
        }
    }
    fn abort(&mut self, transaction: &Transaction) {
        for log in &transaction.compensation_logs {
            match log {
                CompensationLog::Insert(InsertCompensationLog { page_id, slot_id }) => {
    ...
    fn insert(&mut self, transaction: &mut Transaction, tuple: u8) {
        ...
        let page = self.buffer_pool_manager.read_page(page_id);
        {
            let mut page = page.write().unwrap();
            if page.has_space() {
                page.insert_tuple(tuple, transaction);
            } else {
                let new_page = self.buffer_pool_manager.allocate_page();
                let new_page_id = {
                    let mut new_page = new_page.write().unwrap();
                    let new_page_id = new_page.page_id();
                    new_page.insert_tuple(tuple, transaction);
                    new_page_id
                };
                self.buffer_pool_manager.unpin_page(new_page_id);
